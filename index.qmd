...
cashflow_df |>
  ...
  e_tooltip(
    ...
    formatter = htmlwidgets::JS(
      "function(p){
         var amount = Number(p.value[1]); 

         return `
           <div style='font-size:12px; line-height:1.5'>
             <div><b>Label :</b> ${p.name}</div>
             <div><b>Amount :</b> ₹ ${amount.toLocaleString('en-IN')}</div>
           </div>
         `;
       }"
    )
    ...
    axisLabel = list(
      formatter = htmlwidgets::JS(
        "function(value){
          ...
          if (absVal >= 10000000) {
            return sign + (absVal / 10000000) + 'Cr';
          } else if (absVal >= 100000) {
            return sign + (absVal / 100000) + 'L';
          } else if (absVal >= 1000) {
            return sign + (absVal / 1000) + 'K';
          } else {
            return sign + absVal;
          }
        }"
    ...
    formatter = htmlwidgets::JS(
      "function(params){
        ...
        params.forEach(function(p){
          ...
          txt += '<b>' + p.seriesName + '</b><br>' +
                 'Month: ' + month + '<br>' +
                 'Net: ₹' + p.value[1].toLocaleString('en-IN') + '<br><br>';
        });
        return txt;
      }"
    )
...
bank_balance_df %>%
    ...
    axisLabel = list(
      formatter = htmlwidgets::JS(
        "function(value){
          ...
          if (v >= 10000000) return (value / 10000000).toFixed(1) + 'Cr';
          if (v >= 100000)   return (value / 100000).toFixed(1) + 'L';
  ...
  e_tooltip(
    ...
    formatter = htmlwidgets::JS(
      "function(p){
        var amount = Number(p.value[1]);   // ✅ CRITICAL FIX

        return `
          <div style='font-size:12px; line-height:1.6'>
            <div><b>Bank :</b> ${p.name}</div>
            <div><b>Balance :</b> ₹ ${amount.toLocaleString('en-IN')}</div>
          </div>
        `;
      }"
    )
...
txn_movement %>%
  ...
  e_title(
    "Transaction-wise Bank Movement (Each Record)",
    left = "center",
    top = 10,
    textStyle = list(fontSize = 14)
  ) %>%
  ...
  e_tooltip(
    ...
    formatter = htmlwidgets::JS(
      "function(p){
        return '<b>Bank:</b> ' + p.seriesName +
               '<br><b>Date:</b> ' + p.value[0] +
               '<br><b>Amount:</b> ₹ ' +
               p.value[1].toLocaleString('en-IN');
      }"
    )
  ) %>%
  e_legend(

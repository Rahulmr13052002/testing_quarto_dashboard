...
<!-- 🔒 PROTECTED CONTENT -->
<div id="content" style="display:none;" style="margin-top:10px;">
...
bank_table_clean <- bank_table %>%
  mutate(
    books_num = parse_number(`Balance as per Books`),
    bank_num  = parse_number(`Balance as per Bank`),
    cheque_not_yet_recived_num = parse_number(`Cheque issued but not yet presented`),
    cheque_not_yet_recived_num = parse_number(`Cheques Received but not yet deposited`)
  )

books_sum <- sum(bank_table_clean$books_num, na.rm = TRUE)
bank_sum  <- sum(bank_table_clean$bank_num, na.rm = TRUE)
...
reactable(
  ...
  columns = list(
    `Bank Name` = colDef(footer = "Total Current Balance"),

    `Balance as per Books` = colDef(
      align = "right",
      footer = paste0("₹ ", format(books_sum, big.mark = ","))
    ),

    `Balance as per Bank` = colDef(
      align = "right",
      footer = paste0("₹ ", format(bank_sum, big.mark = ","))
    )
  ),
    ...
    paste0("
      function(el) {
        ...
        if (search && !document.getElementById('bank-report-title')) {
          ...
          title.innerHTML = 'Bank Reconciliation Statement as on ", report_date, "';
...
reactable(
  ...
  resizable = TRUE,
  wrap = FALSE,
  striped = TRUE,   # keep striped but override color
  bordered = TRUE,
  elementId = "collections_table",

  # 🔹 Pagination settings
  defaultPageSize = 10,           # show 10 rows per page
  showPageSizeOptions = TRUE,     # allow user to change page size
...
# Total Amount for footer
amount_total <- sum(payables$net_amount, na.rm = TRUE)
...
reactable(
  ...
  columns = list(
    ...
    Amount = colDef(
      ...
      format = colFormat(digits = 2, separators = TRUE),
      footer = function() {
        paste0("₹ ", format(amount_total, big.mark = ","))
      },
      footerStyle = list(
        fontWeight = "bold",
        textAlign = "right"
